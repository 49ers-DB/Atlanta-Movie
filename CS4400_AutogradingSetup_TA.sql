...
-- UserLogin (username, status, isCustomer, isAdmin, isManager)
	...
	IF base = "UserLogin" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', username, status, isCustomer, isAdmin, isManager)");
	END IF;
-- AdFilterUser (username, creditCardCount, userType, status) 
	IF base = "AdFilterUser" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', username, creditCardCount, userType, status)");
	END IF;
-- AdFilterCom (comName, numCityCover, numTheater, numEmployee) 
	IF base = "AdFilterCom" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', comName, numCityCover, numTheater, numEmployee)");
	END IF;
-- AdComDetailTh (thName, thManagerUsername, thCity, thState, thCapacity) 
	IF base = "AdComDetailTh" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', thName, thManagerUsername, thCity, thState, thCapacity)");
	END IF;
-- AdComDetailEmp (empFirstname, empLastname) 
	IF base = "AdComDetailEmp" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', empFirstname, empLastname)");
	END IF;
-- ManFilterTh (movName, movDuration, movReleaseDate, movPlayDate) 
	IF base = "ManFilterTh" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', movName, movDuration, movReleaseDate, movPlayDate)");
	END IF;
-- CosFilterMovie (movName, thName, thStreet, thCity, thState, thZipcode, comName, movPlayDate, movReleaseDate) 
	IF base = "CosFilterMovie" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', movName, thName, thStreet, thCity, thState, thZipcode, comName, movPlayDate, movReleaseDate)");
	END IF;
-- CosViewHistory (movName, thName, comName, creditCardNum, movPlayDate) 
	IF base = "CosViewHistory" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', movName, thName, comName, creditCardNum, movPlayDate)");
	END IF;
-- UserFilterTh (thName, thStreet, thCity, thState, thZipcode, comName) 
	IF base = "UserFilterTh" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', thName, thStreet, thCity, thState, thZipcode, comName)");
	END IF;
-- UserVisitHistory (thName, thStreet, thCity, thState, thZipcode, comName, visitDate) 
	IF base = "UserVisitHistory" THEN
		SET @hash = CONCAT("UPDATE ", i_table, " SET hashing = CONCAT_WS('$', thName, thStreet, thCity, thState, thZipcode, comName, visitDate)");
	END IF;
